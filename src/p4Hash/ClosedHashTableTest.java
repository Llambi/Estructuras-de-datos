...
public class ClosedHashTableTest {
	...
	public void testAdd() {
		...
		// assertEquals(0, tabla2.add(999)); // solo tiene sentido si no hay
		// redispersion
		// assertEquals(17, tabla2.getSize());
		assertEquals(7, tabla2.getSize());
		System.out.println(tabla2.toString());
	...
	public void testRemove() {
		...
		assertEquals(1, tabla.getNumOfElems());
//		assertEquals(5, tabla.getSize()); // dispersion inversa reduce el tamaño de la tabla.
		assertEquals(11, tabla.getSize());
		System.out.println(tabla.toString());
...
//		assertEquals(0, tabla1.remove(0)); // dispersion inversa
		assertEquals(-1, tabla1.remove(0));	// Elemento que no se encontrara en el borrado
		assertEquals(1, tabla1.getNumOfElems());
//		assertEquals(5, tabla1.getSize());
		assertEquals(11, tabla1.getSize());
		System.out.println(tabla1.toString());
	...
	/*
	 * @Test public void testGetLF() {
	 * 
	 * ClosedHashTable<Integer> tabla = new ClosedHashTable<>(3, 0.5, 0.16, (byte)
	 * 0);
	 * 
	 * // REDISPERSION tabla.add(0); assertEquals(3, tabla.getSize());
	 * assertEquals(0.333333333, tabla.getLF(), 0.01);
	 * 
	 * tabla.add(1); assertEquals(7, tabla.getSize()); assertEquals(0.28571,
	 * tabla.getLF(), 0.01);
	 * 
	 * tabla.add(2); tabla.add(3);
	 * 
	 * assertEquals(17, tabla.getSize()); assertEquals(0.23529, tabla.getLF(),
	 * 0.01);
	 * 
	 ...
	 * tabla.remove(3);
	 * 
	 * assertEquals(17, tabla.getSize()); assertEquals(0.176470, tabla.getLF(),
	 * 0.01);
	 * 
	 * tabla.remove(2); assertEquals(7, tabla.getSize()); assertEquals(0.28571,
	 * tabla.getLF(), 0.01);
	 * 
	 * tabla.remove(1); assertEquals(3, tabla.getSize()); assertEquals(0.333333,
	 * tabla.getLF(), 0.01);
	 * 
	 * }
	 * 
	 * @Test public void testFind() { ClosedHashTable<Integer> tabla = new
	 * ClosedHashTable<Integer>(7, 0.5, 0.16, (byte) 0); tabla.add(13);
	 * assertNotNull(tabla.find(13));
	 * 
	 * assertNull(tabla.find(5)); assertNull(tabla.find(null));
	 * 
	 * tabla.add(2); tabla.add(1); tabla.add(4); assertNotNull(tabla.find(2));
	 * assertNotNull(tabla.find(1)); assertNotNull(tabla.find(4)); tabla.remove(2);
	 * assertNull(tabla.find(2));
	 * 
	 * ClosedHashTable<String> string = new ClosedHashTable<String>(7, 0.5, 0.16,
	 * (byte) 0); string.add("A"); assertNotNull(string.find("A"));
	 * 
	 * assertNull(string.find("B")); assertNull(string.find(null));
	 * 
	 * string.add("B"); assertNotNull(string.find("B")); string.add("C");
	 * assertNotNull(string.find("C")); string.add("K");
	 * assertNotNull(string.find("K"));
	 * 
	 * string.remove("B"); assertNull(string.find("B"));
	 * 
	 * }
	 */
